...
/*                    player.KillCount += killCount;
                ...
                case "UPDATE_POSITION":
                    ...
                    // Gửi thông tin vị trí mới đến tất cả người chơi trong cùng lobby
                    BroadcastPlayerPosition(player, direction, x, y);
                    break;
                case "UPDATE_GUN":
                ...
                foreach (var p in lobby.Players)
                {
                    foreach (var otherPlayer in lobby.Players)
                    {
                        if (p != otherPlayer)
                        {
                            ...
                            string positionMessage = $"UPDATE_POSITION;{otherPlayer.PlayerName};{otherPlayer.DirectionTank};{otherPlayer.RectX};{otherPlayer.RectY}";
                            SendMessageToPlayer(p, positionMessage);
                        }
                    }
                }
        ...
        {
            // Nhận giá trị int của Direction
        ...
        private void BroadcastPlayerPosition(PlayerTank player, string direction, int x, int y)
        {
            // Gửi giá trị int của Direction
            string positionMessage = $"UPDATE_POSITION;{player.PlayerName};{(int)player.DirectionTank};{x};{y}";
            Lobby lobby = FindLobbyByPlayer(player);
